...
export const isFromBellville = (regNo) => {
    const bellville = []
...
export const regCheck = (regNo, loc) => {
    if (regNo == "") {
...
export const countRegNumber = regNo => typeof regNo !== 'string' ? "invalid input" : regNo.split(',').length

export const isWeekday = (weekday) => {
...
// ALL FROM TOWN
export const countAllFromTown = (regNo, numPlate) => {
    let res = []
    const result = []
    if (numPlate == '') {
...
//TRANSPORT FEE
export const transportFee = (shift) => {
    if (shift === '') {
        return 'insert value'
...
export const totalPhoneBill = (data) => {
    let total = 0;
...
// ITEMS OVER 20
export const findItemsOver20 = (items) => {
...
// ITEMS OVER
export const findItemsOver = (items, limit) => {
...
export const mostProfitableDepartment = (department) => {
    let salesResult = '';
...
export const mostProfitableDay = (department) => {
    let profitableDay = '';
    let dataListMaxSales = 0;
    department.map(item => {
        if (item.sales >= dataListMaxSales) {
